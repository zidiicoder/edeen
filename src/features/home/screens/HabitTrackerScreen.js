...
export default function HabitTrackerScreen() {
  ...
  const buildHabitPayload = () => {
    ...
    if (frequency === 'Custom') {
      return {
        ...basePayload,
        frequency: 'custom',
        custom_date: (habitCustomDate || habitStartDate).toISOString().split('T')[0],
      };
    }
  ...
  const onToggleDay = async (habit, dayIndex) => {
    ...
    if (isFutureDate(completionDate)) {
      showUserMessage('You cannot mark a future date.');
      return;
    }
          ...
          {habitsLoading ? (
            ...
            habits.map(habit => {
              ...
              <View
                ...
                style={[
                  styles.habitCard,
                  { backgroundColor: habit.color || '#E8CDBA' },
                ]}
              >
                <View style={styles.habitHeader}>
                  ...
                  <TouchableOpacity
                    style={[
                      styles.currentDayCheckBtn,
                      isCurrentDayChecked && styles.currentDayCheckBtnActive,
                      (!isCurrentDayAvailable || isCurrentDayUpdating) && styles.currentDayCheckBtnDisabled,
                    ]}
                    onPress={() =>
                      isCurrentDayAvailable
                        ? onToggleDay(habit, currentDayIndex)
                        : null
                    }
                    disabled={!isCurrentDayAvailable || isCurrentDayUpdating}
                    activeOpacity={0.85}
                  >
                    {isCurrentDayChecked ? (
                      <Feather name="check" size={16} color="#0B7A2A" />
                    ) : (
                      <View style={styles.currentDayCheckInner} />
                    )}
                  </TouchableOpacity>
                </View>
                ...
                >
                  <View style={styles.daysRow}>
                    ...
                    <View style={styles.daysRightWrap}>
                      <Text style={styles.daysTextRight}>
                ...
                <View style={styles.habitGridWrap}>
                  {Array.from({
                    length: Math.ceil(getTotalBoxes(habit) / 10),
                  }).map((_, rowIndex) => {
                    const total = getTotalBoxes(habit);
                    const rowStart = rowIndex * 10;
                    const rowEnd = Math.min(rowStart + 10, total);
                      ...
                      >
                        <View style={styles.rowBoxes}>
                          {Array.from({ length: countInRow }).map((__, i) => {
                            ...
                            const checked = isDayChecked(habit, dayIndex);
                            const isUpdatingDay = Boolean(
                            ...
                            return (
                              <TouchableOpacity
                                ...
                                onPress={() => onToggleDay(habit, dayIndex)}
                ...
                <View style={styles.habitFooterRow}>
                  <View style={styles.metaRow}>
                    ...
                    <View style={styles.metaPillSoft}>
                      <Text style={styles.metaPillTextSoft}>
