...
import { getAppStartDate, localYMD } from '../../../utils/appDate';
...
// Days from account creation date back to today, capped at 40 days.
async function getHistoryDays(user) {
  const days = [];
  const today = new Date();
  today.setHours(0, 0, 0, 0);

  // Get account creation date from user object or fallback to AsyncStorage
  ...
  // Earliest visible day = max(accountCreatedDate, today - 39).
  const fortyAgo = new Date(today);
  fortyAgo.setDate(today.getDate() - (MAX_HISTORY_DAYS - 1));
  const earliest = accountCreatedDate > fortyAgo ? accountCreatedDate : fortyAgo;

  const cursor = new Date(today);
  while (cursor >= earliest) {
    const dateKey = localYMD(cursor);
    days.push({
      date: dateKey,
...
export default function PrayerHistoryScreen({ navigation }) {
  ...
  const [salahRecordsMap, setSalahRecordsMap] = useState({});
  ...
  const fetchHistory = async () => {
    try {
      ...
      // Fetch all salah records at once
      const response = await request({ url: 'salah/records', method: 'GET' });
      const records = response?.data?.salah_records || [];
      ...
      setSalahRecordsMap(recordsMap);
      
      // Get history days from account creation
      const days = await getHistoryDays(user);
      
      // Map days to records
      const historyWithRecords = days.map(day => ({
        ...day,
        record: recordsMap[day.date] || {},
      }));
      
      setHistoryData(historyWithRecords);
    } catch (error) {
      ...
      >
        <View style={styles.summaryCard}>
          <Text style={styles.summaryTitle}>
            {historyData.length >= MAX_HISTORY_DAYS
              ? 'Last 40 Days'
              : `Your ${historyData.length} Day${historyData.length === 1 ? '' : 's'}`}
          </Text>
          <View style={styles.summaryStats}>
            <View style={styles.summaryStatItem}>
              <Text style={styles.summaryStatValue}>
                {historyData.filter(d => d.record?.tahajud_performed).length}
              </Text>
              <Text style={styles.summaryStatLabel}>Tahajjud</Text>
            </View>
            <View style={styles.summaryStatItem}>
              <Text style={styles.summaryStatValue}>
