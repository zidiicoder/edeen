...
const INFO_SLIDES = [
  {
    image: require('../../../assets/images/edeen-logo.png'),
    badge: '#FBEAAF',
      ...
      >
        {!isQuizStep ? (
          <View style={styles.slide}>
            <View style={[
              step === 0 ? styles.iconWrapWelcome : styles.iconWrap,
              step === 0 ? null : { backgroundColor: slide.badge }
            ]}>
              <Image 
                source={slide.image} 
                style={step === 0 ? styles.slideImageWelcome : styles.slideImage}
                resizeMode="contain"
              />
            </View>
...
const styles = StyleSheet.create({
  ...
  iconWrap: {
    // Big round badge; the icon inside is kept within the circle (a square only
    // fits a circle up to ~70% of its diameter, so 104 sits inside 176).
    width: 176,
    height: 176,
    borderRadius: 88,
    alignItems: 'center',
